...
# setup.sh — Universal setup script for thunderbird-template-reloaded
#
# Detects your shell, platform, and installs prerequisites.
# Then hands off to `just setup` for project-specific configuration.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hyperpolymath/thunderbird-template-reloaded/main/setup.sh | sh
#   # or after cloning:
#   ./setup.sh
#
...
# ── Main ──
main() {
    printf "%s=== thunderbird-template-reloaded Setup ===%s\n\n" "$BOLD" "$RESET"
    ...
    if [ ! -f "Justfile" ] && [ ! -f "justfile" ]; then
        ...
        info "Clone first: git clone https://github.com/hyperpolymath/thunderbird-template-reloaded.git"
        info "Then: cd thunderbird-template-reloaded && ./setup.sh"
        exit 1
    fi
